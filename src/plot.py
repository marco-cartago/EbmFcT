...
def show_single_sample(x: torch.Tensor, title: str = None, cmap: str = "Grays"):
    """
    Visualize a single PyTorch image [C,H,W] or [H,W].
    """
    x = x.detach().cpu()

    # if x is a batch of images, take the first one
    if x.ndim == 4:
        x = x[0]
...
def compare_real_fake(real: torch.Tensor, fake: torch.Tensor, cmap="Grays"):
    """
    Show a side-by-side comparison of real and fake images.
    """
...
def visualize_heads(
    ...
    """
    Visualize some samples generated by each head of the model.
    """
